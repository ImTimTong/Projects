...
//Postcondition: Assigns a copy of a source deque to current deque 
    // ASSIGNMENT OPERATOR
    template <class Item>
...
//Postcondition: Frees dynamic memory of deque object. 
    //DESTRUCTOR
    template <class Item>
    deque<Item>::~deque () {
	...
	{
		if (block_pointers[bp_array_index] != NULL)
...
//Postcondition: Frees memory for data blocks 
    template <class Item>
    void deque<Item>::clear () {
        ...
        for(size_type bp_array_index = 0; bp_array_index < bp_array_size; ++bp_array_index)
	{
		if(block_pointers[bp_array_index] == NULL) continue;
...
//Postcondition: Adds new item to front of deque. 
    template <class Item>
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // STUDENT WORK...
            value_type *new_block = new value_type[block_size];
	    new_block[0] = entry;
...
//Postcondition: Adds new item to back of deque
    template <class Item>
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            // STUDENT WORK...
	    size_t idx = floor(block_size/2); //start insert at middle
            value_type *new_block = new value_type[block_size];
	    new_block[idx] = entry;
	    *last_bp = new_block; //add 1st block
	    front_ptr = back_ptr = new_block+idx; //point to entry
        }
	    ...
	    *(++back_ptr) = entry; //incr back_ptr
	    ...
	    value_type *new_block = new value_type[block_size];
	    new_block[0] = entry;
	    *(++last_bp) = new_block; //incr last_bp
	    back_ptr = new_block; //back_ptr at 1st entry
        }
...
//Precondition: Deque must not be empty
//Postcondition: Removes item at front of deque. 
    template <class Item>
    void deque<Item>::pop_front()
    {
        ...
        {
            // STUDENT WORK...
	    delete [] *first_bp; //remove first block
	    *first_bp = NULL;
	    first_bp++; //go to next block
	    front_ptr = *first_bp; //update front_ptr
        }
...
//Precondition: Deque must not be empty.
//Postcondition: Removes item at back of deque. 
    template <class Item>
    void deque<Item>::pop_back()
    {
        assert(!isEmpty());
        
        if (back_ptr == front_ptr) //if one elt in deque
        {
            clear( ); //reset deque
        }
        else if (back_ptr == *last_bp) //if 1st elt of last block
        {
            // STUDENT WORK...
	    delete [] *last_bp; //remove last block
	    *last_bp = NULL;
	    last_bp--; //go to prev block
	    back_ptr = *last_bp + block_size - 1; //update back_ptr
        }
        else //decrement pointer if in the middle
        {
            --back_ptr;
        }
    }
