...
//     Item argument (using the Item type from the node).
    ...
    {
        if (node_ptr != NULL) //skip if node null
        {
            // STUDENT WORK
            inorder(f, node_ptr->left( ) ); //applies f in inorder
            f( node_ptr->data( ) );
...
//     Item argument (using the Item type from the node).
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (node_ptr != NULL) //skip if node null
        {
            postorder(f, node_ptr->left( ) ); //apply f in postorder
...
//     Item argument (using the Item type from the node).
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (node_ptr != NULL) //skip if node null
        {
            f( node_ptr->data( ) ); //apply f in preorder
...
//     heap, and root_ptr has been set to NULL.
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (root_ptr != NULL) //skip if node null
        {
            child = root_ptr->left( );
            tree_clear( child );
            
            // STUDENT WORK
            child = root_ptr->right( ); //clear right side
            tree_clear( child );
            delete root_ptr; //remove root
            root_ptr = NULL;
        }
    }
...
//     value is a pointer to the root of this copy.
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (root_ptr == NULL) //copy NULL
            return NULL;
        else
        {
            // STUDENT WORK
            l_ptr = tree_copy( root_ptr->left( ) ); //copy LHS
            r_ptr = tree_copy( root_ptr->right( ) ); //copy RHS
            return new binary_tree_node<Item>(root_ptr->data( ), l_ptr, r_ptr); //connect root
        }
    }
...
//     Postcondition: The return value is the number of nodes in the tree.
    ...
    // Library facilities used: cstdlib
    {
        if (node_ptr == NULL) //if NULL, return zero nodes
            return 0;
        else //recursively add num nodes
        {
            // STUDENT WORK
            size_t size = 1 + tree_size(node_ptr->left()); //add root and LHS num nodes
            size += tree_size(node_ptr->right()); //add RHS num nodes
            return size; //return num nodes
        }
    }
}
