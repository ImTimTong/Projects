...
//Postcondition: Returns true if iterators are equal, otherwise false
        bool operator ==(const deque_iterator other) const
        {	
		//MY WORK
		return (block_pointers == other.block_pointers && block_pointers_end == other.block_pointers_end
...
//Postcondition: Increments iterator and then returns it 
        ...
        deque_iterator& operator ++( ) // Prefix ++
        {
            ...
            if (cursor == current_boundary) //reached end of current block
            {
		//MY WORK
		++current_block_pointer; //incr current_block ptr and cursor
		cursor = *current_block_pointer;
		current_boundary = *current_block_pointer + block_size -1; //set end for new block
            }

            // Move forward
            else //if cursor in middle of a block
            {
                //MY WORK
		++cursor; //incr cursor
            }
...
//Postcondition: Increment iterator and return a copy of it before it was incremented
		...
		++cursor; //incr cursor
            ...
            return original; //return original iterator
        }
